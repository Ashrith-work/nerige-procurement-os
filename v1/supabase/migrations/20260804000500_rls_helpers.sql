-- =============================================================================
-- M1 / 005 — RLS helper functions
-- =============================================================================
-- The most security-critical objects in the system. Every isolation policy in
-- every future milestone resolves through these three functions.
--
...
--     evaluates them once per query rather than once per row. On a 50k-row
--     table that is the difference between 3 ms and 3 s.
--
...
--
-- The functions derive everything from auth.uid(). No caller-supplied vendor_id
-- is ever trusted, anywhere.
-- =============================================================================
...
-- how access revocation takes effect within one request rather than one hour.
...
-- The vendor organisation the caller belongs to, or NULL for internal staff.
-- Reads the join table, so a vendor with three logins gets one shared island.
...
-- True for any authenticated member of staff (founder, procurement head,
-- warehouse manager). False for vendors and for anonymous callers.
...
as $$
  select coalesce(app.current_role() in
    ('founder', 'procurement_head', 'warehouse_manager'), false);
$$;

-- Roles permitted to create and amend procurement master data.
create or replace function app.can_manage_vendors()
returns boolean
language sql
stable
security definer
...
-- Convenience for the vendor side of every policy: true when the caller is a
-- vendor user AND the row belongs to their organisation. Written as one
-- function so the isolation predicate exists in exactly one place — a policy
-- that hand-rolls `vendor_id = app.current_vendor_id()` risks omitting the
-- NULL guard, and `vendor_id = NULL` is NULL (not true), but relying on that
-- subtlety across dozens of future policies is how leaks happen.
