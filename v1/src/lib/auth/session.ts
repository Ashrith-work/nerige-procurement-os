...
import { DEMO_USERS, isDemoRole } from '@/lib/demo/procurement'

/** Cookie marking a demo session. Meaningless unless demo mode is enabled. */
export const DEMO_COOKIE = 'nerige_demo'
...
export const getSessionUser = cache(async (): Promise<SessionUser | null> => {
  // Demo mode short-circuits authentication entirely. isDemoMode() is hard-off
  // in production, so this branch cannot execute on a deployed instance.
  //
  // The cookie carries WHICH role is being demonstrated, because the point of
  // the walkthrough is that the three people see three different systems. A
  // bare '1' is the original single-account form and still resolves, so an
  // existing demo session is not broken by this.
  if (isDemoMode()) {
    const jar = await cookies()
    const value = jar.get(DEMO_COOKIE)?.value
    if (!value) return null
    if (value === '1') return DEMO_USER
    return isDemoRole(value) ? DEMO_USERS[value] : null
  }
