...
describe('storage object isolation', () => {
  ...
  it('allows a vendor their own document objects and nothing else', async () => {
    // Asserted as "every visible object is mine" rather than a fixed count, so
    // adding a document kind to the fixtures (a bill photo, a dispatch image)
    // strengthens this test instead of breaking it.
    const rows = await db.asUser(w.vendorB.ownerUser, async (c) => {
      const { rows } = await c.query<{ name: string }>(`select name from storage.objects`)
      return rows
    })
    expect(rows.length).toBeGreaterThan(0)
    expect(rows.filter((r) => !r.name.includes(w.vendorB.id))).toEqual([])
  })
