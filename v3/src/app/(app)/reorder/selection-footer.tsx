 ...
 *
 * INSIDE THE ORDERING FLOW it carries the flow on its link and is named for the
 * step it opens rather than for the end of the job: "Send to 3 vendors" on a
 * screen where nothing is sent yet is the kind of promise that teaches people to
 * stop reading buttons. Dropping `?flow=order` here would end the flow at the
 * step that most needs its trail.
 */
export function SelectionFooter({ flow = false }: { flow?: boolean }) {
  ...
  return (
    <div className="no-print fixed inset-x-0 bottom-0 z-40 border-t border-stone-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center gap-3 px-4 py-3">
        ...
        <Link
          href={flow ? '/reorder/review?flow=order' : '/reorder/review'}
        ...
        >
          {flow ? 'Say how many' : `Send to ${vendors} ${vendors === 1 ? 'vendor' : 'vendors'}`}
        </Link>
      </div>
    </div>
  )
